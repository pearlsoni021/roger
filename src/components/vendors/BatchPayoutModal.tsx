      ...
      <div className="w-full max-w-lg rounded-xl border border-[#E2DFD8] bg-[#111726] p-6 shadow-xl">
        <div className="flex items-center justify-between border-b border-[#E2DFD8] pb-3.5">
          <div>
            ...
            <p className="text-xs text-[#5E6C84]">Direct Escrow NEFT/RTGS with Penny-Drop Verified Beneficiaries</p>
          </div>
        ...
        <div className="mt-4 space-y-3.5 text-xs">
          <div className="rounded-lg border border-[#E2DFD8] bg-[#0D131F] p-3.5">
            <div className="flex justify-between text-[#5E6C84] pb-1.5 border-b border-[#E2DFD8]">
              <span>Total Invoices in Batch:</span>
              <span className="font-semibold text-[#1C2331] font-mono">{payableInvoices.length} Invoices</span>
            </div>
            <div className="flex justify-between font-semibold text-xs pt-1.5">
              <span className="text-[#1C2331]">Total Net Disbursement:</span>
          ...
          <div className="space-y-1">
            ...
            <div className="h-1.5 w-full rounded-full bg-[#F7F6F2] overflow-hidden">
              <div
          ...
          <div className="rounded-lg border border-[#E2DFD8] bg-[#0D131F] p-3 max-h-44 overflow-y-auto space-y-2 font-mono">
            {payableInvoices.map((inv, idx) => (
              <div key={inv.id} className="flex items-center justify-between text-[11px] border-b border-slate-900 pb-1.5 last:border-0">
                <div>
                  <div className="text-[#1C2331] font-sans font-medium">{inv.vendorName}</div>
                  <div className="text-[10px] text-[#5E6C84]">{inv.invoiceNumber} | TDS: {formatINR(inv.tdsAmountINR)}</div>
                </div>
                <div className="text-right">
                  <div className="text-slate-100 font-bold">{formatINR(inv.netPayableINR)}</div>
                  <div className="text-[9px] text-[#5E6C84]">
          ...
          <div className="flex items-center gap-2 text-[11px] text-[#5E6C84] bg-[#0D131F] p-2.5 rounded border border-[#E2DFD8]">
            <ShieldCheck className="h-4 w-4 text-emerald-500 shrink-0" />
            <span>Form 26Q TDS ledger updated and payment advices dispatched.</span>
          </div>

          <div className="flex justify-end pt-2 border-t border-[#E2DFD8]">
            <button
              onClick={onClose}
              disabled={!isDone}
