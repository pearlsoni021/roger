...
export const ReconciliationStudio: React.FC<ReconciliationStudioProps> = ({
  ...
  const getStatusBadge = (status: MatchStatus, resolutionStatus: string) => {
    if (resolutionStatus === 'RESOLVED') {
      return (
        <span className="inline-flex items-center gap-1.5 rounded-md border border-[#EAEAEA] bg-[#F5F0E6] px-2 py-0.5 text-[11px] font-medium text-[#2E2E2E]">
    ...
    if (resolutionStatus === 'FLAGGED_UNRESOLVED' || status === 'UNRESOLVED_EXCEPTION') {
      return (
        <span className="inline-flex items-center gap-1.5 rounded-md border border-purple-900/50 bg-[#F9F3EB] px-2 py-0.5 text-[11px] font-medium text-purple-300">
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 rounded-xl border border-[#EAEAEA] bg-[#FFFFFF] p-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-[#2E2E2E]">3-Way Financial Reconciliation Studio</h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {discrepancies.length > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-[#EAEAEA] bg-[#F5F0E6] px-3 py-1.5 text-xs font-medium text-[#2E2E2E] hover:bg-[#EBE5DE] hover:text-[#2E2E2E] transition-colors"
            >
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        <div className="rounded-lg border border-[#EAEAEA] bg-[#F5F0E6] p-3">
          <div className="text-[10px] font-medium text-[#6B6B6B] uppercase">Batch Match Rate</div>
          <div className="text-base font-bold text-[#2E2E2E] font-mono mt-1">{metrics.batchMatchRatePercent}%</div>
          <div className="text-[10px] text-[#6B6B6B] mt-0.5">{matchedCount}/{records.length} records verified</div>
        </div>

        <div className="rounded-lg border border-[#EAEAEA] bg-[#F5F0E6] p-3">
          <div className="text-[10px] font-medium text-[#6B6B6B] uppercase">Total Audited Volume</div>
          <div className="text-base font-bold text-[#2E2E2E] font-mono mt-1">{formatINR(records.reduce((s, r) => s + r.gatewayGrossINR, 0), { compact: true })}</div>
          <div className="text-[10px] text-[#6B6B6B] mt-0.5 font-mono">{metrics.throughputRecordsPerSec} rec/sec</div>
        </div>

        <div className="rounded-lg border border-[#EAEAEA] bg-[#F5F0E6] p-3">
          <div className="text-[10px] font-medium text-[#6B6B6B] uppercase">Fee Drift Variances</div>
          <div className="text-base font-bold text-[#2E2E2E] font-mono mt-1">{discrepancies.length} Items</div>
          <div className="text-[10px] text-[#6B6B6B] mt-0.5 font-mono">1-click auto-balance</div>
        </div>

        <div className="rounded-lg border border-[#EAEAEA] bg-[#F5F0E6] p-3">
          <div className="text-[10px] font-medium text-[#6B6B6B] uppercase">Settlement Timing Float</div>
          <div className="text-base font-bold text-[#2E2E2E] font-mono mt-1">{timingLags.length} Pending</div>
          <div className="text-[10px] text-[#6B6B6B] mt-0.5">T+2 clearing window</div>
        </div>

        <div className="rounded-lg border border-purple-900/40 bg-[#F9F3EB] p-3">
      ...
      {/* Filter Tabs */}
      <div className="flex items-center gap-1.5 border-b border-[#EAEAEA] pb-2 text-xs overflow-x-auto">
        <button
          onClick={() => setFilterTab('ALL')}
          className={`rounded-md px-2.5 py-1 font-medium transition-colors ${
            filterTab === 'ALL' ? 'bg-[#F5F0E6] text-[#2E2E2E]' : 'text-[#6B6B6B] hover:text-[#2E2E2E]'
          }`}
        >
        ...
        <button
          ...
          className={`rounded-md px-2.5 py-1 font-medium transition-colors ${
            filterTab === 'DISCREPANCIES' ? 'bg-[#F5F0E6] text-[#2E2E2E]' : 'text-[#6B6B6B] hover:text-[#2E2E2E]'
          }`}
        >
        ...
        <button
          ...
          className={`rounded-md px-2.5 py-1 font-medium transition-colors ${
            filterTab === 'TIMING_LAG' ? 'bg-[#F5F0E6] text-[#2E2E2E]' : 'text-[#6B6B6B] hover:text-[#2E2E2E]'
          }`}
        >
        ...
        <button
          ...
          className={`rounded-md px-2.5 py-1 font-medium transition-colors ${
            filterTab === 'MATCHED' ? 'bg-[#F5F0E6] text-[#2E2E2E]' : 'text-[#6B6B6B] hover:text-[#2E2E2E]'
          }`}
        >
      ...
      <div className="rounded-xl border border-[#EAEAEA] bg-[#FFFFFF] overflow-hidden">
        <div className="overflow-x-auto max-h-[520px]">
          <table className="w-full text-left text-xs">
            <thead className="sticky top-0 border-b border-[#EAEAEA] bg-[#FFFFFF] text-[11px] font-medium text-[#6B6B6B] uppercase tracking-wider z-10">
              <tr>
              ...
              {filteredRecords.map((rec) => (
                ...
                >
                  <td className="py-3 px-3.5">
                    <div className="font-semibold text-[#2E2E2E] group-hover:text-[#4A6982]">{rec.id}</div>
                    <div className="text-[10px] text-[#6B6B6B] font-sans">{rec.orderId}</div>
                  </td>
                  <td className="py-3 px-3.5 text-[#2E2E2E]">
                  ...
                  <td className="py-3 px-3.5 text-[#6B6B6B] font-sans">
                    {rec.invoiceNumber ? (
                      <span className="font-mono text-[#6B6B6B]">{rec.invoiceNumber}</span>
                    ) : (
                      <span className="text-[#6B6B6B] text-[11px]">Unrecorded</span>
                    )}
                  </td>
                  <td className="py-3 px-3.5 font-bold">
                    <span className={rec.varianceINR === 0 ? 'text-[#6B6B6B]' : 'text-[#2E2E2E]'}>
                  ...
                  <td className="py-3 px-3.5 text-right font-sans">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded border border-[#EAEAEA] bg-[#F5F0E6] px-2 py-1 text-[11px] font-medium text-[#6B6B6B] hover:bg-[#EBE5DE] hover:text-[#2E2E2E]"
                    >
                      <span>Review</span>
