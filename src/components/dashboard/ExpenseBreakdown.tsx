    ...
    <div className="rounded-3xl bg-white border border-slate-300 p-6 shadow-sm flex flex-col justify-between h-full min-h-[320px]">
      ...
      <div>
        <div className="flex items-center justify-between pb-3">
          <button className="flex h-7 w-7 items-center justify-center rounded-full text-slate-700 hover:bg-slate-100 font-bold">
          ...
          {days.map((d) => (
            <div
              ...
              className={`flex flex-col items-center justify-center py-2.5 rounded-2xl transition-all ${
                d.isSelected
                  ? 'bg-[#242831] text-white shadow-md'
                  : 'text-slate-700 hover:bg-slate-100'
              }`}
            >
              <span className={`text-[11px] font-bold ${d.isSelected ? 'text-slate-200' : 'text-slate-600'}`}>
                {d.day}
              </span>
              <span className={`text-xs font-black mt-1 ${d.isSelected ? 'text-white' : 'text-slate-900'}`}>
                {d.date}
              </span>
            </div>
          ))}
      ...
      <div className="mt-4 pt-4 border-t border-slate-200 flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 flex items-center gap-1 text-xs font-extrabold text-emerald-700">
        ...
        <div className="relative flex items-center justify-center h-12 w-12 rounded-full border-4 border-slate-200 bg-white shadow-inner">
