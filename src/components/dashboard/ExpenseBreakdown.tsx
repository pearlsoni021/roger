          ...
          {days.map((d) => (
            <div
              ...
              className={`flex flex-col items-center justify-center py-2.5 rounded-2xl transition-all ${
                d.isSelected
                  ? 'bg-[#2D323B] text-white shadow-md'
                  : 'text-slate-600 hover:bg-slate-50'
              }`}
            >
              <span className={`text-[10px] font-semibold ${d.isSelected ? 'text-slate-200' : 'text-slate-400'}`}>
                {d.day}
              </span>
              <span className={`text-xs font-bold mt-1 ${d.isSelected ? 'text-white' : 'text-slate-800'}`}>
                {d.date}
              </span>
            </div>
          ))}
      ...
      <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
        ...
        <div className="relative flex items-center justify-center h-12 w-12 rounded-full border-4 border-slate-100 bg-white shadow-inner">
          <svg className="absolute inset-0 h-full w-full -rotate-90" viewBox="0 0 36 36">
            <path
              className="text-[#2D323B] stroke-current"
              strokeWidth="3.5"
